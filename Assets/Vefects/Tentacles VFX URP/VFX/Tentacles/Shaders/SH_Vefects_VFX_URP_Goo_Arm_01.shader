...
Shader "Vefects/SH_Vefects_VFX_URP_Goo_Arm_01"
{
	Properties
	{
		...
		_TargetPosition0("Target Position 0", Vector) = (0,0,0,0)
_TargetPosition1("Target Position 1", Vector) = (0,0,0,0)
_TargetPosition2("Target Position 2", Vector) = (0,0,0,0)
_TargetPosition3("Target Position 3", Vector) = (0,0,0,0)


		_AttractionRadius("Attraction Radius", Float) = 1
		_AttractionStrength("Attraction Strength", Float) = 1
		_AttractionMaskPower("Attraction Mask Power", Float) = 1
	...
	SubShader
	{
		...
		Pass
		{
			...
			float _AttractionMaskPower;

			float4 _TargetPosition0;
			float4 _TargetPosition1;
			float4 _TargetPosition2;
			float4 _TargetPosition3;
			...
			PackedVaryings VertexFunction( Attributes input )
{
	...
	float2 texCoord10 = input.texcoord.xy;
	float2 panner68 = ( _Time.y * _NoiseTexturePan + ( texCoord10 * _NoiseTextureScale ));
	float2 texCoord70 = input.texcoord.xy;
	float2 panner72 = ( _Time.y * _NoiseTextureDistortionPan + ( texCoord70 * _NoiseTextureDistortionScale ));
	float lerpResult61 = lerp( -1.0 , 1.0 , tex2Dlod( _NoiseTextureDistortion, float4( panner72, 0, 0 ) ).g);
	float2 lerpResult63 = lerp( float2(0,0) , lerpResult61.xx , _NoiseDistortionLerp);
	float2 noiseUV = panner68 + lerpResult63;

	float4 noiseSample = tex2Dlod( _NoiseTexture, float4( noiseUV, 0, 0 ) );
	float wpoNoise = lerp( _WPOMin , _WPOMax , noiseSample.g ) * _WPOIntensity1;

	float3 wpoNormal = input.normalOS * wpoNoise;

	float y = input.texcoord.y;
	float bandMask = saturate(smoothstep(
		_WPOBandErosion,
		_WPOBandErosion + _WPOBandErosionSmoothness,
		saturate(pow(abs(1.0 - y) * abs(y), 1.0) * 3.0)
	));

	float sineOffset = sin(_TimeParameters.x * _WPOSineTime) * _WPOSineIntensity * bandMask;
	float3 wpo = float3(wpoNormal.x, wpoNormal.y + sineOffset, wpoNormal.z);

	float collapseMask = saturate((1.0 - y) + lerp(-1.0, 1.0, _WPOCollapseLerp));
	float3 finalWPO = lerp(input.positionOS.xyz * _WPOCollapseWidth, wpo, collapseMask);

	input.positionOS.xyz += finalWPO;

	VertexPositionInputs vertexInput = GetVertexPositionInputs(input.positionOS.xyz);
	float3 positionWS = vertexInput.positionWS;

	float tipMask = saturate(pow(y, 2.0));

	
	float3 tp0 = _TargetPosition0.xyz;

	float d0 = distance(positionWS, tp0);

	float m0 = saturate(1.0 - d0 / _AttractionRadius);

	float3 dir = tp0 - positionWS;
	float dist = length(dir);

	float radius = _AttractionRadius;
	float arrivalRadius = radius * 0.25;

	float t = saturate(dist / radius);
	float a = saturate(dist / arrivalRadius);

	float force = (1.0 - t) * a;
	force *= force;

	float3 totalAttraction = normalize(dir) * force;

	float maxDisplacement = radius * 0.2;
	float len = length(totalAttraction);
	totalAttraction = totalAttraction / max(len, 0.0001) * min(len, maxDisplacement);

	

	float3 attractionWS = totalAttraction * _AttractionStrength * tipMask;

	float4 clipOffset =
		TransformWorldToHClip(positionWS + attractionWS)
		- vertexInput.positionCS;

	vertexInput.positionCS += clipOffset;
